...
Quaternion Matrix4x4::GetRotation(const Matrix4x4& m)
{
	...
	if (trace > 0.0f)
	{
		float s = WaveMath::SqrF(trace + 1.0f) * 2.0f;
		q.w = 0.25f * s;
		q.x = (m.m21 - m.m12) / s;
		q.y = (m.m02 - m.m20) / s;
		q.z = (m.m10 - m.m01) / s;
	...
	{
		float s = WaveMath::SqrF(1.0f + m.m00 - m.m11 - m.m22) * 2.0f;
		q.w = (m.m21 - m.m12) / s;
		q.x = 0.25f * s;
		q.y = (m.m01 + m.m10) / s;
		q.z = (m.m02 + m.m20) / s;
	...
	{
		float s = WaveMath::SqrF(1.0f + m.m11 - m.m00 - m.m22) * 2.0f;
		q.w = (m.m02 - m.m20) / s;
		q.x = (m.m01 + m.m10) / s;
		q.y = 0.25f * s;
		q.z = (m.m12 + m.m21) / s;
	...
	{
		float s = WaveMath::SqrF(1.0f + m.m22 - m.m00 - m.m11) * 2.0f;
		q.w = (m.m10 - m.m01) / s;
		q.x = (m.m02 + m.m20) / s;
		q.y = (m.m12 + m.m21) / s;
		q.z = 0.25f * s;
